...
I would like to note that one major tradeoff in the language is there
is very limited parenthesis after invokation and the language is
motivated and powered by Rich Hickey rolling in his hammock.
...
** Clojure Language Interoperability and Data Representation
   ...
   primitive functions exposed from the clojure ecosystem. *fif* works
   off of EDN (Extensible Data Notation) value types.
...
* Extending fif within Clojure
  ...
  amazing extensibility between *fif* and clojure. Ignoring n-arity
  within clojure, there is a wealth of functionality that can be
  easily included in *fif* with very little code.
...
** fif and clojure interoperability
   ...
   of evaluators over clojure data subject to the same clojure reader
   shortfalls that prevent it from being used as a data format.

   *Note that reading in data in string representation does not suffer
   from these shortfalls as discussed in a later section*
