...
[[http://benzaporzan.me/fif-playground/][Try it Online!]] *In Alpha*
...
*fif is currently in alpha*. *Some language features are likely to change*.
...
* Installation
  ...
  *Clojure CLI/deps.edn*
  ...
  fif {:mvn/version "0.4.0-SNAPSHOT"}
  ...
  *Gradle*
  ...
  compile 'fif:fif:0.4.0-SNAPSHOT'
...
** Variables
   ...
   *fif* strays away from Forth in the way it sets and gets
   variables. Since *Word Referencing* was added to *fif*, the ability
...
** fif and clojure interoperability
   ...
   *Note that reading in data as a string representation does not
   ...
   through an escape sequence, '%=. This would be useful when
   generating code from a client to plug into a fif stack machine as a
   server command.
